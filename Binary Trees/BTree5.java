...
class BTree{
    ...
    public static void main (String[] args) {
        ...
        int numberOfNodes = findSize(rootNode);
        System.out.println("size of the tree is : "+numberOfNodes);
        
        /*second tree*/
        System.out.println("second tree");
        rootNode = createBTree2();
        size = findSize(rootNode);
    ...
    static int findSize(Node node){
        ...
        if(node.leftChild == null && node.rightChild == null)
            return 1;
        int size = findSize(node.leftChild) + findSize(node.rightChild) + 1;
        return size;
    }
}
